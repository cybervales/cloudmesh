...
func reportMetrics() {
	for {
		...
		m := Metric{
			...
			CPUUsage:    15.0 + rand.Float64()*10.0,
			MemoryUsage: 45.0 + rand.Float64()*5.0,
			Connections: rand.Intn(100),
			Status:      status,
		}
...
func handleProxy(w http.ResponseWriter, r *http.Request) {
	...
	chaos.mu.RLock()
	if chaos.LatencyActive { time.Sleep(time.Duration(500+rand.Intn(1500)) * time.Millisecond) }
	isError := chaos.ErrorActive && rand.Float64() > 0.5
	chaos.mu.RUnlock()
	...
	authReq, _ := http.NewRequest("GET", "http://localhost:8082/validate", nil)
	authReq.Header.Set("X-Trace-ID", traceID)
	client := &http.Client{Timeout: 5 * time.Second}
	resp, err := client.Do(authReq)

	if err != nil || resp.StatusCode != http.StatusOK {
		http.Error(w, "Auth Failure", http.StatusServiceUnavailable)
		return
	}
	...
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, "Gateway: Request %s Processed", traceID)
}
...
func main() {
	...
	fmt.Println("Gateway Service starting on :8081 (HARDENED)...")
